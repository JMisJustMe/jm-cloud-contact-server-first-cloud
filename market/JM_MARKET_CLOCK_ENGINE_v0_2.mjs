...
function alignedChanges(rowsA,rowsB,sourceTransform,targetTransform){
 ...
 return {changes:out,commonCount:common.length,medianCadenceMs:cadence};
}
...
export function quantifyClock(rowsA,rowsB,opts={}){
 ...
 const {changes,commonCount,medianCadenceMs}=alignedChanges(rowsA,rowsB,sourceTransform,targetTransform);
 ...
 return {
  schema:'JM.MismatchedClockQuantifier/0.2',
  ...
  threshold,frictionBps,medianCadenceMs,observedLagMs,observedLagHours:Number.isFinite(observedLagMs)?observedLagMs/3600000:null,clockBand:clockBand(observedLagMs),mismatch:best.lag>0,
