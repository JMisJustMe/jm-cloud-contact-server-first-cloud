...
function csp(type,nonce=''){const script=nonce?`script-src 'self' 'nonce-${nonce}'`:"script-src 'self'";return type.startsWith('text/html')?`default-src 'none'; style-src 'self' 'unsafe-inline'; ${script}; connect-src 'self'; img-src 'self' data:; base-uri 'none'; frame-ancestors 'none'; form-action 'none'`:"default-src 'none'; frame-ancestors 'none'"}
function send(req,res,code,body,type='application/json; charset=utf-8',nonce=''){const raw=typeof body==='string'?body:JSON.stringify(body);res.writeHead(code,{...cors(req),'Content-Security-Policy':csp(type,nonce),'Content-Type':type,'Content-Length':Buffer.byteLength(raw)});res.end(raw)}
function sendPublic(req,res,file,type){try{let raw=fs.readFileSync(path.join(PUBLIC_DIR,file),'utf8');let nonce='';if(type.startsWith('text/html')){nonce=b64(crypto.randomBytes(18));raw=raw.replaceAll('<script>',`<script nonce="${nonce}">`)}return send(req,res,200,raw,type,nonce)}catch{return send(req,res,404,{ok:false,error:'public asset not found'})}}
